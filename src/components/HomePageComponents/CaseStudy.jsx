...
// Sub-component to handle individual number rolling
const RollingNumber = ({ value, isActive }) => {
    const [displayValue, setDisplayValue] = useState(0);
    const obj = { val: 0 };

    // Extract number and suffix (e.g., "40" and "%")
...
const CaseStudyScroll = () => {
    ...
    const [activeIndex, setActiveIndex] = useState(-1); // Start at -1 so first card triggers on enter
    ...
    useLayoutEffect(() => {
        ...
        // Trigger for index switching and stats restart
    ...
    return (
        <div ref={containerRef} className="relative flex flex-col lg:flex-row bg-black min-h-screen">
            ...
            <div className="w-full lg:w-1/2 px-10 md:px-24">
                {projects.map((item, i) => (
                    <div key={item.id} className={`case-section-${i} min-h-screen flex flex-col justify-center py-32`}>
                        <div className="flex items-center gap-4 mb-6">
                            {/* LOGO: Uses the ID-based initials or custom logo URL provided in JSON */}
                            <div className="w-12 h-12 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-xs   font-bold text-white overflow-hidden">
                                {item.logo.includes('http') ? (
                                    <img src={item.logo} alt="" className="w-full h-full object-cover opacity-50" />
                        ...
                        <h2 className="text-4xl md:text-6xl font-black text-white leading-none mb-6 tracking-wider ">
                            {item.company}
                        </h2>
                        ...
                        {/* STATS ROW: Mapping the results object into the 3-column grid */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-10 border-y border-white/10 mb-10">
                            {/* Result 1: Revenue/Capital */}
                            <div className="flex flex-col">
                                <span className="text-[12px]  tracking-wider text-zinc-500 mb-2 font-bold ">Impact</span>
                                <span className="text-xl md:text-2xl font-black text-white tracking-wider">
                                    {item.results.revenueGrowth}
                                </span>
                            </div>
                            {/* Result 2: Leads/Meetings */}
                            <div className="flex flex-col">
                                <span className="text-[12px]  tracking-wider text-zinc-500 mb-2 font-bold ">Acquisition</span>
                                <span className="text-xl md:text-2xl font-black text-white tracking-wider">
                                    {item.results.leadGen}
                                </span>
                            </div>
                            {/* Result 3: Highlight Metric */}
                            <div className="flex flex-col">
                                <span className="text-[12px]  tracking-wider text-zinc-500 mb-2 font-bold ">Key Result</span>
                                <span className="text-xl md:text-2xl font-black text-white tracking-wider">
            ...
            <div ref={rightSideRef} className="hidden lg:flex w-1/2 h-screen sticky top-0 items-center justify-center">
                <div className="relative w-full h-full overflow-hidden bg-zinc-900 border-l border-white/10">
                    ...
                    ${activeIndex === i ? "opacity-100 scale-100" : "opacity-0 scale-125 tranzinc-y-10"}`}
                        >
                            <img src={item.image} className="w-full h-full object-cover grayscale-[0.3] brightness-50" alt={item.company} />
                            <div className="absolute inset-0 bg-gradient-to-br from-zinc-500/20 to-black/60 mix-blend-multiply" />
